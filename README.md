...
内置 **牧濑红莉栖 (Amadeus)** ，傲娇天才科学家与你实时互动！

## 功能

### 🤖 微信平台
- ✅ **自动监控与回复** - 定时检测微信消息，自动回复
- ✅ **消息方向识别** - 区分自己发送的消息和对方发送的消息，只对对方消息触发回复，避免自我触发
- ✅ **多联系人支持** - 同时监控多个联系人，支持 Web 面板勾选启用/禁用
- ✅ **活跃联系人锁定** - 检测到某个联系人发消息后锁定该联系人，持续对话，避免多联系人频繁切换窗口
- ✅ **联系人管理** - 支持添加/删除联系人，配置关系描述
- ✅ **工作时间段** - 可配置自动回复的时间范围
- ✅ **静默期** - 每个联系人可配置静默时间，期间不回复
- ✅ **自定义回复** - 每个联系人可配置首次自定义回复语句

### 🧠 大模型驱动
- ✅ **多提供商支持** - 豆包、OpenAI、DeepSeek、通义千问、Kimi、智谱 GLM 等
- ✅ **角色扮演模式** - 内置牧濑红莉栖角色，傲娇天才科学家设定
...
- ✅ **日志查看** - 浏览回复历史记录
- ✅ **联系人勾选** - 在 Web 面板中勾选要监控的联系人
- ✅ **启动/停止控制** - 在 Web 面板中控制微信聊天模块的启停
- ✅ **Live2D 交互** - 内置牧濑红莉栖 Live2D 模型，支持表情切换、动作播放、对话

### 🎨 Live2D 交互
- ✅ **3D 模型渲染** - 基于 Cubism 5 的 Live2D 模型
- ✅ **表情切换** - 支持脸红、思考、微笑、眨眼、惊讶、生气、悲伤等多种表情
- ✅ **动作播放** - 多种预设动画（挥手、撩发、歪头等）
- ✅ **鼠标追踪** - 模型视线跟随鼠标移动（可开关）
- ✅ **对话气泡** - Live2D 对话气泡，支持打字机效果
- ✅ **说话口型同步** - 说话时嘴部动画自动同步
...
| `llm.character` | 角色名称（当前支持 `kurisu` 和 `kurisu_live2d`） | `kurisu` |
...
# 指定主机和端口（默认 127.0.0.1:5000）
python main.py --web-admin --web-host 0.0.0.0 --web-port 8080
```

启动后浏览器访问 `http://localhost:5000` 即可打开管理面板。

> **注意**：Web 模式下，微信聊天模块默认暂停，需在 Web 面板中点击「启动」按钮开启。
...
管理面板提供直观的图形界面，包含以下页面：

- **仪表盘** - 运行状态、回复统计、启动/停止控制
- **配置管理** - 在线编辑检测间隔、LLM 参数、工作时间段等
- **联系人管理** - 添加/删除联系人，勾选要监控的联系人
- **Live2D 交互** - 与牧濑红莉栖实时对话，支持表情切换和动作播放
- **日志查看** - 浏览回复历史记录
...
```
Amadeus/
├── main.py              # 主程序入口（命令行参数解析、Flask Web 服务、主循环）
├── config.py            # 配置管理模块（加载/保存/验证配置）
├── wechat_monitor.py    # 微信窗口监控模块（UI Automation 控制）
├── auto_reply.py        # 自动回复引擎模块（消息检测、LLM 调用、回复发送）
├── llm_client.py        # 大模型客户端模块（支持多提供商）
├── character_prompts.py # 角色扮演提示词模块
├── personalization.py   # 个性化配置模块（静默期、自定义回复等）
├── config.json          # 默认配置文件
...
│   └── voice/           #   语音平台
├── web/                 # Web 管理面板（Flask）
│   ├── app.py           #   Flask 应用（API 路由、Live2D 对话）
│   ├── templates/       #   HTML 模板
│   │   └── index.html   #   单页应用（仪表盘/配置/联系人/Live2D/日志）
│   └── static/          #   静态资源
│       ├── lib/         #   Live2D Cubism 5 SDK 库文件
│       └── live2d/      #   Live2D 模型文件（model3.json、motions/等）
...
2. 查找聊天列表中的指定联系人
3. 模拟点击激活聊天窗口
4. 获取对方最新消息内容（通过 ClassName 和控件特征区分自己发送和对方发送的消息）
5. 调用大模型 API 生成角色扮演回复
6. 模拟键盘输入并发送回复
7. 定时循环检测，实现实时对话

### 消息方向识别
通过分析微信聊天列表的 UI 控件结构来区分自己发送的消息和对方发送的消息：

- **自己发送的消息**：ClassName 包含 "Self"、子控件中按钮数量 ≥ 2（撤回等操作按钮）、AutomationId 包含 "self"
- **对方发送的消息**：不具备上述特征

这样确保只对对方的消息触发自动回复，避免自己发送的回复消息再次触发检测循环。

### 多联系人锁定策略
当监控多个联系人时，采用"活跃联系人锁定"策略避免频繁切换窗口：

1. 检测到某个联系人发消息后，锁定该联系人为"活跃联系人"
2. 持续检测该联系人的新消息并回复（对方发一条回一条）
3. 直到该联系人没有新消息，才解锁并检查其他联系人
4. 如果当前聊天窗口已经是目标联系人，跳过窗口切换操作
...
### Q: 运行报错 `ValueError: I/O operation on closed file`
A: 这是 Flask Web 服务接管 stdout 导致的问题。已在 `main.py` 中通过 `_safe_print()` 函数和 stdout 恢复机制修复。如果仍然遇到，请确保使用最新版本。

### Q: 多个联系人之间来回切换窗口
A: 已通过"活跃联系人锁定"策略和"当前窗口检测"优化解决。程序会优先检测当前活跃联系人，且如果当前聊天窗口已经是目标联系人，会跳过窗口切换操作。

### Q: 如何添加新的联系人？
A: 两种方式：
1. 编辑 `config.json` 的 `contacts` 数组，添加 `{"name": "备注名", "relation": "关系"}`
2. 在 Web 管理面板的「联系人」页面中点击「添加联系人」
